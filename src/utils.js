...
const generateChartData = ({ items, keyToGroup, keyOfValue, aliases, type, backgroundColor }) => {
  const grouped = groupBy(items, keyToGroup, idx)
  const response = {}

  for (let key in grouped) {
    response[(aliases && aliases[key]) || key] =
      grouped[key].reduce((accumulated, item) => accumulated + item[keyOfValue], 0)
  }
